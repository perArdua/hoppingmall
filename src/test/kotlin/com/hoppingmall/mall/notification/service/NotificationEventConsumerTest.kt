...
class NotificationEventConsumerTest {
    ...
    fun `결제 완료 알림 이벤트를 처리한다`() {
        ...
        // when
        notificationEventConsumer.handleNotificationEvent(event, "1")
        ...
        assertEquals(123L, savedNotification.orderId)
        assertEquals(456L, savedNotification.paymentId)
    ...
    fun `포인트 적립 알림 이벤트를 처리한다`() {
        ...
        val event = NotificationEvent(
            ...
            orderId = 123L,
            paymentId = 456L,
            pointAmount = BigDecimal("100")
        )
        
        val captor = argumentCaptor<Notification>()
        
        // when
        notificationEventConsumer.handleNotificationEvent(event, "2")
        ...
        assertEquals(123L, savedNotification.orderId)
        assertEquals(456L, savedNotification.paymentId)
        assertEquals(BigDecimal("100"), savedNotification.pointAmount)
        assertEquals(false, savedNotification.isRead)
    }
} 